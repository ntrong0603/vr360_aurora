...
@section('title', 'Sửa thông tin mục đích thăm viếng')
@section('content')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">Quản lý mục đích thăm viếng</a></li>
        ...
        <div class="container-fluid">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title title-custom">SỬA THÔNG TIN MỤC ĐÍCH THĂM VIẾNG</h3>
                </div>
                <!-- /.card-header -->
                <form class="form-horizontal form-brand" action="{{ route('visiting.update', ['visiting' => $visiting->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                            ...
                            <div class="tab-pane fade {{$key == 0 ? 'active show' : ''}}" id="lang-{{$language->code}}" role="tabpanel" aria-labelledby="lang-{{$language->code}}-tab">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        @if($visitingLanguage->lang == $language->code)
                                        @php
                                        $notIssetLang = false;
                                        @endphp
                                        <input type="text" class="form-control @error('name_'.$language->code) is-invalid @enderror" name="name_{{$language->code}}" value="{{old('name_'.$language->code, $visitingLanguage->name)}}">
                                        @endif
                                        @endforeach
                                        @if($notIssetLang)
                                        <input type="text" class="form-control @error('name_'.$language->code) is-invalid @enderror" name="name_{{$language->code}}" value="{{old('name_'.$language->code, $visiting->name)}}">
                                        @endif
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('visiting.index') }}" class="btn btn-info">Quay lại</a>
                    </div>
                </form>
            </div>
        </div><!-- /.container-fluid -->
