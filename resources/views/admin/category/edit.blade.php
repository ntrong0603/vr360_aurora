        ...
        <div class="container-fluid">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title title-custom">SỬA THÔNG TIN DANH MỤC</h3>
                </div>
                <!-- /.card-header -->
                <form class="form-horizontal form-brand" action="{{ route('category.update', ['category' => $category->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <select class="form-control @error('name_scene') is-invalid @enderror" name="name_scene" id="">
                                    <option value="">Chọn cảnh liên kết</option>
                                    @foreach ($scenes as $scene)
                                    <option value="{{$scene->name_scene}}" {{ old('name_scene', $category->name_scene)==$scene->name_scene ? 'selected' : '' }}>{{$scene->name}}</option>
                                    @endforeach
                                </select>
                                @error('name_scene')
                                <span class="error invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-2">
                                <div class="icheck-success d-inline">
                                    <input type="checkbox" id="checkboxStatus" value="1" name="status" {{ (!old() || old('status', $category->status)) ? 'checked' : '' }}>
