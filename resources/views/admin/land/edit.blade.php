...
@section('title', 'Sửa thông tin lô đất')
@section('content')
        ...
        <div class="container-fluid">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title title-custom">SỬA THÔNG TIN LÔ ĐẤT</h3>
                </div>
                <!-- /.card-header -->
                <form class="form-horizontal form-brand" action="{{ route('land.update', ['land' => $land->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <select class="form-control @error('status') is-invalid @enderror" name="status" id="" value="{{old('status')}}">
                                    <option value="1" {{ old('status', $land->status)==1 ? 'selected' : '' }}>Còn trống</option>
                                    <option value="2" {{ old('status', $land->status)==2 ? 'selected' : '' }}>Đang giữ chỗ</option>
                                    <option value="3" {{ old('status', $land->status)==3 ? 'selected' : '' }}>Đã cho thuê</option>
                                </select>
                                @error('status')
                                <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <select class="form-control @error('style') is-invalid @enderror" name="style" id="" value="{{old('style')}}">
                                    <option value="1" {{ old('style', $land->style)==1 ? 'selected' : '' }}>Lô đất</option>
                                    <option value="2" {{ old('style', $land->style)==2 ? 'selected' : '' }}>Nhà xưởng xây sẵn</option>
                                </select>
                                @error('style')
                                <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
