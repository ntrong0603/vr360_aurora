...
@section('title', 'Sửa thông tin loại doanh nghiệp')
@section('content')
        ...
        <div class="container-fluid">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title title-custom">SỬA THÔNG TIN LOẠI DOANH NGHIỆP</h3>
                </div>
                <!-- /.card-header -->
                <form class="form-horizontal form-brand" action="{{ route('businessStyle.update', ['businessStyle' => $businessStyle->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                            ...
                            <div class="tab-pane fade {{$key == 0 ? 'active show' : ''}}" id="lang-{{$language->code}}" role="tabpanel" aria-labelledby="lang-{{$language->code}}-tab">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        @if($businessStyleLanguage->lang == $language->code)
                                        @php
                                        $notIssetLang = false;
                                        @endphp
                                        <input type="text" class="form-control @error('name_'.$language->code) is-invalid @enderror" name="name_{{$language->code}}" value="{{old('name_'.$language->code, $businessStyleLanguage->name)}}">
                                        @endif
                                        @endforeach
                                        @if($notIssetLang)
                                        <input type="text" class="form-control @error('name_'.$language->code) is-invalid @enderror" name="name_{{$language->code}}" value="{{old('name_'.$language->code, $businessStyle->name)}}">
                                        @endif
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-2">
                                <div class="icheck-success d-inline">
                                    <input type="checkbox" id="checkboxStatus" value="1" name="status" {{ (old('status', $businessStyle->status)) ? 'checked' : '' }}>
